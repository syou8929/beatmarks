...
- 将来の一般配布・販売を視野に入れた依存ライセンス方針(2026-07-12 承認、詳細は `docs/license-comparison.md`):
  - **GPL / AGPL 系(aubio、essentia、madmom 等)は使用禁止**(自社コード公開義務が生じるため)
  - **LGPL は、動的リンクまたは別プロセス実行で差し替え可能な形に限り許容**する。該当: ffmpeg(別プロセス)、libsndfile(soundfile 経由)、soxr(librosa のリサンプラ)
  - その他の実行時依存は許諾系(MIT / BSD / ISC / Apache-2.0)のみ
  - `THIRD_PARTY_LICENSES.md` を自動生成し、LGPL 部分のライセンス文書とソース入手先を明記する
  - **ビルド専用ツール**は成果物にコピーレフトが及ばないものに限り GPL 系も許容(例: PyInstaller — 商用配布を許可する特別例外付き)
  - 注: Mac App Store 配布は LGPL と相性が悪いため、必要になった時点で別途検討(直接販売系チャネルには影響なし)
