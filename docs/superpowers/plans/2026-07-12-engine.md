...
- ライセンス方針(2026-07-12 承認): **GPL/AGPL 禁止**(aubio・essentia・madmom 等)。**LGPL は動的リンク/別プロセスで差し替え可能な形なら許容**(libsndfile・soxr・ffmpeg が該当)。その他の実行時依存は MIT/BSD/ISC/Apache のみ。ビルド専用ツールは成果物にコピーレフトが及ばなければ可(PyInstaller 承認済み)
