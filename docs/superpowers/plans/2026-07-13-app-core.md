...
> **レビュー後の修正(2026-07-13 適用済み、commits e6f1491 + a2ed67b)**: ①**計画コードのバグ(実装者が発見・レビューで実証)**: 本文の UNDO/REDO は「現在アクティブなソース」にスナップショットを復元するが、本計画自身のテスト(SOURCE_SWITCHEDはundo対象外)は「編集時にアクティブだったソース」への復元を要求 → `UndoEntry {sourceId, edits}` タグ付けで解決(サンドボックスで計画コード素のまま実行し 6/7 失敗を確認済み) ②undo上限100境界(最古が脱落)と「undo後の新編集でredoクリア」のテスト追加 ③GRID_KEYS に hitThreshold を含めない理由コメント(hit IDは解析配列添字で安定、beat-/bar-/sil-は位置再計算だからクリア) ④両 Action switch に never 網羅性ガード(15アクション明示)。テスト 192→194。
> **設計メモ**: undo/redoはソース横断の単一時系列スタック(sourceIdタグ付き) — Cmd+Z が非アクティブソースの編集を戻すことがある。UI(③b)はその際アクティブタブ切替 or トースト表示を検討。ANALYZE_FAILED/エラーフェーズは AppState に未定義(③bで導入検討)。
...
> **レビュー後の修正(2026-07-13 適用済み、commits 7481d71 + 527c2bf)**: 全て計画コード由来のバグ。①**Critical**: 長さクランプ/onended欠如 — 自然終了後も playing=true のまま currentTime() が無限に伸び、次の play() が offset≥duration で無音再生(UI上は再生中・復旧不能)。currentTime() を duration でクランプ+srcNode.onended(現行ノード同一性ガード付き)+終端からの play() は 0 から再開 ②ループ判定が `!metronome` の早期returnの下にありメトロノームOFFだと setLoop が完全に不活性 → pump() を「ループ判定(playing時常時)」と「クリックスケジュール(metronome時のみ)」に分離 ③React.StrictMode の dev 二重マウントで dispose 後の遅延 load() が新 AudioContext を生成しリーク → disposed フラグで load() をガード(dispose冪等化) ④onDrop の probeMedia が未ガードで拒否時にドロップ画面で無言スタック → try/catch+alert ⑤BrowserWindow に backgroundThrottling: false(ブラー中のメトロノーム欠落防止) ⑥ctxFactory 注入でフェイクAudioContext状態機械テスト6件(各修正のリバートで失敗することを確認済み=非空虚)。テスト 198→204。
> **③b持ち越し**: シーク時に発火済みクリックosc(~300ms分)が旧位置のまま鳴る化粧的グリッチ(スクラバー実装時に pending click 参照を stop(0)) / AudioContext.resume() の明示呼び出しは未実装(Chromiumのsticky activation依存) / App.tsx の sources.find(...)! 非nullアサーションは③bで防御化。
...
> **重要: 本計画の処方箋は無効だった(2026-07-13、commits 363762b + 5dc4590 で別解を適用)**: 本文の「判定を backtrack 前拍列の diff-CV に切替」は数値的に無効果 — cv_diff(raw)=cv_diff(beats)=0.022941(6桁一致、opusレビューで追試確認)。真因は librosa beat_track の DP 自体のフレーム量子化(120bpm は 21.53 フレーム/拍の非整数で±1交互)であり backtrack ではない。採用した別解: **線形フィット残差CV**(拍時刻を拍番号の一次式にフィット、CV=残差の頑健スケール/フィット周期)。旧 diff-CV に対し全レジームで優越(固定20曲中10曲の誤variable解消・退行0・+4bpm/60sの実ドリフト161msを旧統計は fixed と誤判定/新統計は正しく variable)。さらにフォローアップで残差スケールを**両端トリムstd**(最初と最後の拍の残差を除外)にロバスト化+閾値0.02→0.025(140bpm/8sのスプリアス端拍対策。worst_fixed=0.0230 vs worst_drift=0.0326 でナイフエッジなし)。境界回帰テスト3件(140bpm/8s・140bpm/30s offset0.6・174bpm/30s)。エンジンテスト 53→59。variable時の beatConfidence は設計上0近くに潰れ low-beat-confidence 警告が強ドリフトで発火(意図的、docstring記載)。constraints.txt/README は計画通り。
...
> **レビュー後の修正(2026-07-13 適用済み、commits a51508c + 4a1b144)**: 本文実装は全項目レビュー通過(ゴールデン21件不変を独立確認)。フォローアップ: ①`timeSigDenominatorFor(beatsPerBar)` ヘルパを types.ts に追加(6→8、他→4。**③b の writeExports は必ずこれで ExportContext を組むこと** — ハードコード4だと6/8のMIDI拍子が無音で誤る) ②midi.ts に分母バリデーション({2,4,8,16,32} 以外は ExportError — log2(12)=silent garbage 対策) ③reaper/audacity \r テストを出力内容断言に強化(行数断言は修正前後で判別不能だった — レビューが実証) ④cc=24/bb=8 恒定の意図コメント。テスト 211→214。
> **計画②持ち越しバンドルの未消化2件の処置(レビュー指摘)**: (a)「validate深化」→ ③bへ正式スケジュール(validateProjectFile の sources要素/ui.fps 形チェックと合流、Task 6 注記済み) (b)「JSONゴールデンのエンベロープ分離検討」→ ③bの writeExports 実装時にゴールデン再編とあわせて判断(それまで保留)。
